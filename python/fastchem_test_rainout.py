...
i = 19
pressure = p_t_grid[0,i,1]

print(pressure)


min_temp = np.min(p_t_grid[:,i,0])
      
#create a temperature structure that goes from very high temperatures to the lowerst one
temperatures = np.arange(6200, min_temp, -50.0)
temperatures = np.append(temperatures, min_temp)
      
#add all temperature points from the grid if they are not already included
for temp in p_t_grid[:,i,0]:
  if np.any(np.where(temperatures == temp)) == False:
    temperatures = np.append(temperatures, temp)
      
#and sort the vector in decending order
temperatures[::-1].sort()

#create the pressure vector for the FastChem input
pressures_fastchem = np.full(temperatures.shape, pressure)


temperature = temperatures
pressure = pressures_fastchem
...
#temperature =  np.atleast_1d(temperature[12])
#pressure =  np.atleast_1d(pressure[12])
...
fastchem.setParameter('condIterChangeLimit', 1.0)
...
fastchem.setParameter('nbSwitchToNewton', 1000)
# fastchem.setParameter('accuracyNewton',1e-8)
fastchem.setParameter('accuracyCond',1e-10)
fastchem.setParameter('nbIterationsChemCond',900000)
